...
async function handle({ request }: { request: Request }) {
  ...
  try {
    ...
    const { data: dueSoon } = await supabaseAdmin
      .from("tasks")
      .select(
        "id,title,description,priority,due_date,assignee_id,assignee_name,reminder_24h_sent_at",
      )
    ...
    const { data: overdue } = await supabaseAdmin
      .from("tasks")
      .select(
        "id,title,description,priority,due_date,assignee_id,assignee_name,created_by,overdue_notified_at",
      )
