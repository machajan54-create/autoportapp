...
async function handle({ request }: { request: Request }) {
  ...
  try {
    ...
    const { data: vyk } = await supabaseAdmin
      .from("vykupy")
      .select("id,znacka,model,klient,poznamka,follow_up_at,internal_priced_by_user_id")
    ...
    for (const v of vyk ?? []) {
      ...
      if (!recipient.email) {
        ...
        await supabaseAdmin.from("vykupy").update({ follow_up_notified_at: nowIso }).eq("id", v.id);
        continue;
      }
    ...
    for (const d of deals ?? []) {
      ...
      if (!recipient.email) {
        await supabaseAdmin.from("deals").update({ follow_up_notified_at: nowIso }).eq("id", d.id);
        continue;
      }
      await enqueueTransactionalEmail({
        templateName: "followup-reminder",
