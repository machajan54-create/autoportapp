...
function FeedbackPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (trimmed.length < 1) {
      setErrMsg("Prosím napište zprávu.");
      return;
    }
    if (trimmed.length > 2000) {
      setErrMsg("Zpráva je příliš dlouhá (max 2000 znaků).");
      return;
    }
    setStatus("sending");
    setErrMsg(null);
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <span
        style={{ fontSize: 12, opacity: 0.65, textTransform: "uppercase", letterSpacing: "0.1em" }}
      >
        {label}
      </span>
      {children}
