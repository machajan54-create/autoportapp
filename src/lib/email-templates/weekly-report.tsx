      ...
      <Container style={styles.container}>
        ...
        <Section style={styles.content}>
          ...
          <Section style={card}>
            ...
            <Row
              label="Z toho kritické"
              value={fmt(defectsCritical)}
              highlight={defectsCritical > 0}
            />
          </Section>
              ...
              <>
                ...
                {dppWarnings.map((w) => (
                  <Row
                    key={w.name}
                    label={w.name}
                    value={`${w.hours.toFixed(1)} / 300 h`}
                    highlight={w.hours >= 300}
                  />
                ))}
              </>
            )}
...
export const template = {
  component: Email,
  subject: ({ periodLabel }: WeeklyReportProps) =>
    `Autoport APP — ${periodLabel ?? "Týdenní přehled"}`,
