      ...
      <Container style={styles.container}>
        ...
        <Section style={styles.content}>
          <Heading as="h1" style={styles.h1}>
            Změna fáze obchodního případu
          </Heading>
          <Text style={styles.lead}>
            {recipientName ? `Ahoj ${recipientName}, ` : ""}
            {actorName || "Kolega"} posunul(a) případ{" "}
            {fromStageLabel ? `z „${fromStageLabel}" ` : ""}
            do fáze „{toStageLabel}".
          </Text>
          <Section style={styles.metaBox}>
            <Text style={styles.metaRow}>
              <strong>Případ:</strong> {title || "—"}
            </Text>
            {vehicle ? (
              <Text style={styles.metaRow}>
                <strong>Vůz:</strong> {vehicle}
              </Text>
            ) : null}
            {clientName ? (
              <Text style={styles.metaRow}>
                <strong>Klient:</strong> {clientName}
              </Text>
            ) : null}
            {durationLabel ? (
              <Text style={styles.metaRow}>
                <strong>Doba v předchozí fázi:</strong> {durationLabel}
              </Text>
            ) : null}
          </Section>
