...
const Email = ({ recipientName = '', orderNumber = '', modelVerze = '', orderUrl, invoiceUrl }: Props) => (
        ...
        <Section style={styles.content}>
          ...
          {orderUrl ? (
            <Text style={styles.metaRow}>
              📄 <Link href={orderUrl} style={{ color: '#2563eb' }}>Stáhnout podepsanou objednávku</Link>
            </Text>
          ) : null}
          {invoiceUrl ? (
            <Text style={styles.metaRow}>
              🧾 <Link href={invoiceUrl} style={{ color: '#2563eb' }}>Stáhnout zálohovou fakturu</Link>
            </Text>
          ) : null}
        </Section>
        <Footer />
...
export const template = {
  ...
  previewData: { recipientName: 'Jan Novák', orderNumber: 'OBJ-2026-0001', modelVerze: 'C5 Aircross', orderUrl: 'https://example.com/o.pdf', invoiceUrl: 'https://example.com/f.pdf' },
