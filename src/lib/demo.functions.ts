...
export const ensureDemoUser = createServerFn({ method: "POST" }).handler(async () => {
    ...
    await supabaseAdmin.from("claims").insert([
      {
        ...
        address: "Korunní 12, Praha 2",
        insurer: "ČSOB Pojišťovna",
        claim_number: "C-2026-0001",
        event_at: new Date(Date.now() - 1000 * 60 * 60 * 24 * 3).toISOString(),
        location: "Praha 4, Jižní spojka",
        liquidation_type: "Totální škoda",
        vat_payer: "Ne",
        notes: "Demo zakázka — čelní střet, vyřizuje se s pojišťovnou.",
        signature:
      ...
      },
      {
        ...
        email: "p.svobodova@example.cz",
        address: "Masarykovo nám. 3, Brno",
        insurer: "Allianz",
        claim_number: "C-2026-0002",
        event_at: new Date(Date.now() - 1000 * 60 * 60 * 24 * 10).toISOString(),
        location: "D1, exit 178",
        liquidation_type: "Oprava",
        vat_payer: "Ano",
        notes: "Demo — boční náraz, čeká se na díly.",
        signature:
          "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=",
        status: "new",
      },
      {
        company: "AutoLogistik s.r.o.",
        ico: "27889922",
        first_name: "Tomáš",
        last_name: "Dvořák",
        phone: "+420 731 222 111",
        email: "dvorak@autologistik.cz",
        address: "Průmyslová 8, Plzeň",
        insurer: "Kooperativa",
        claim_number: "C-2026-0003",
        event_at: new Date(Date.now() - 1000 * 60 * 60 * 24 * 30).toISOString(),
        location: "Plzeň, Borská pole",
        liquidation_type: "Oprava",
        vat_payer: "Ano",
        notes: "Demo — dokončeno, vyplaceno.",
        signature:
          "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=",
        status: "done",
      },
    ...
    await supabaseAdmin.from("vykupy").insert([
      {
        ...
        stav: "Vykoupeno",
        zpracoval: "Demo účet",
        poznamka: "Demo — připraveno k prodeji.",
      },
      {
        znacka: "Volkswagen",
        ...
        stav: "Prodáno",
        zpracoval: "Demo účet",
        poznamka: "Demo — prodáno se ziskem.",
      },
      {
        znacka: "BMW",
        model: "320d xDrive",
        ...
        stav: "Nacenění",
        zpracoval: "Demo účet",
        poznamka: "Demo — čeká na osobní prohlídku.",
      },
    ]);
  }
  ...
  if (!suppliersCount) {
    const { data: sup } = await supabaseAdmin
      ...
      .insert([
        {
          name: "AutoDíly Express s.r.o.",
          ico: "28456789",
          dic: "CZ28456789",
          contact_person: "Ing. Pavel Marek",
          email: "marek@autodily-express.cz",
          phone: "+420 234 567 890",
          address: "Logistická 5, Praha 9",
          notes: "Demo dodavatel originálních dílů.",
          status: "approved",
        ...
        {
          name: "LakovnaPro s.r.o.",
          ico: "29887766",
          dic: "CZ29887766",
          contact_person: "Jiří Hruška",
          email: "info@lakovnapro.cz",
          phone: "+420 555 111 222",
          address: "Průmyslová 14, Ostrava",
          notes: "Demo lakovna — spolupráce na rámcové smlouvě.",
          status: "pending",
          requested_by: user.id,
        },
      ])
    ...
    await supabaseAdmin.from("purchases").insert([
      {
        ...
        decided_by: user.id,
        decided_at: new Date().toISOString(),
        decision_note: "Schváleno v rámci limitu.",
      },
      {
      ...
      },
      {
        ...
        decided_by: user.id,
        decided_at: new Date().toISOString(),
        decision_note: "Odloženo na další kvartál.",
      },
    ]);
  }
    ...
    if (emps && shifts) {
      ...
      for (let day = 1; day <= 5; day++) {
        ...
        emps.slice(0, 4).forEach((e, i) => {
          ...
          checkIn.setHours(baseHour, Math.floor(Math.random() * 10), 0, 0);
          const checkOut = new Date(d);
          checkOut.setHours(baseHour + 8, 30 + Math.floor(Math.random() * 20), 0, 0);
          records.push({
            employee_id: e.id,
            shift_id: shiftId,
            date: dateStr,
            ...
            break_duration: 30,
            hours_worked: 8.0 + Math.random() * 0.5,
          });
        });
      }
      ...
      await supabaseAdmin.from("attendance_absences").insert([
        {
          ...
          end_date: new Date(Date.now() + 11 * 86_400_000).toISOString().slice(0, 10),
          note: "Demo — letní dovolená",
          status: "pending",
        },
        {
          ...
          end_date: new Date(Date.now() + 2 * 86_400_000).toISOString().slice(0, 10),
          note: "Demo — kontrola",
          status: "approved",
          resolved_at: new Date().toISOString(),
          resolved_by: emps[0].id,
        },
        {
          employee_id: emps[5].id,
          type: "nemoc",
          start_date: new Date(Date.now() - 3 * 86_400_000).toISOString().slice(0, 10),
          end_date: new Date(Date.now() - 1 * 86_400_000).toISOString().slice(0, 10),
          note: "Demo — chřipka",
          status: "approved",
          resolved_at: new Date().toISOString(),
          resolved_by: emps[0].id,
      ...
      await supabaseAdmin.from("attendance_notifications").insert([
        {
          ...
          title: "🏝️ Žádost o dovolenou: Kolář Michal",
          message: "[Hlavní Provoz] Nová žádost o dovolenou čeká na schválení.",
          is_for_manager: true,
          meta: { employee_id: emps[3].id },
        },
        {
          type: "late_arrival",
          title: "⏱️ Opožděný příchod: Bálek Jakub",
          message: "[Hlavní Provoz] Pozdní příchod o 12 minut.",
          is_for_manager: true,
          meta: { employee_id: emps[4].id },
        },
      ]);
    }
  }
