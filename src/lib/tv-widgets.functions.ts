...
export type TvWidgetResult =
  | { widget: 'stats'; day: { vykupy: number; prodano: number; ukoly_done: number }; week: { vykupy: number; prodano: number } }
  ...
  .handler(async ({ data }): Promise<TvWidgetResult> => {
    ...
    if (data.widget === 'stats') {
      const [vDay, vWeek, prodDay, prodWeek, ukoly] = await Promise.all([
        supabaseAdmin.from('vykupy').select('id', { count: 'exact', head: true }).gte('created_at', startOfDay.toISOString()),
        supabaseAdmin.from('vykupy').select('id', { count: 'exact', head: true }).gte('created_at', startOfWeek.toISOString()),
        supabaseAdmin.from('vykupy').select('id', { count: 'exact', head: true }).eq('stav', 'Prodáno').gte('stav_changed_at', startOfDay.toISOString()),
        supabaseAdmin.from('vykupy').select('id', { count: 'exact', head: true }).eq('stav', 'Prodáno').gte('stav_changed_at', startOfWeek.toISOString()),
        supabaseAdmin.from('tasks').select('id', { count: 'exact', head: true }).eq('status', 'done').gte('updated_at', startOfDay.toISOString()),
      ])
      return {
        widget: 'stats',
    ...
    if (data.widget === 'vehicles') {
      ...
      if (list.length) {
        const { data: ph } = await supabaseAdmin
          ...
          .in('vykup_id', list.map((r: any) => r.id))
        ...
        if (paths.length) {
          const { data: signed } = await supabaseAdmin.storage.from('vykup-photos').createSignedUrls(paths, 60 * 60 * 6)
    ...
    if (data.widget === 'news') {
      ...
      return { widget: 'news', items: items.map((r: any) => ({ id: r.id, title: r.title, body: r.body })) }
    }
    ...
    if (data.widget === 'weather') {
      ...
      const lon = 16.6068
      try {
        const res = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weather_code&timezone=Europe%2FPrague`)
